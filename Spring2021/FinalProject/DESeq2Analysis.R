...
# Preparing .rnk file with ALL genes (not just significant p<0.05)
all_genes_ranked = results_with_adjustment2[order(results_with_adjustment2$log2FoldChange),c("gene_symbol", "log2FoldChange")]

write.table(all_genes_ranked, sep = "\t", file = "/Users/jaredkhan/Desktop/Lee Lab/all_genes.rnk", row.names=FALSE, col.names=FALSE, quote=FALSE)
